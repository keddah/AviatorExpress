...
class AeroPhysics
{
    ...
    public static float GetBladeArea(float radius)
    {
    ...
    public static float GetAirDensity(float altitude)
    {
        ...
        float pressureAtAltitude = seaLvlPressure *
                                   Mathf.Pow((tempAtAltitude / seaLvlTemp),
                                       (-Physics.gravity.y / (deltaTemp * gasConstant)));
    ...
    public static float FindWingAreaPerSection(float wingSpan = 14, float rootChord = 1.8f, float tipChord = 1.5f,
        float wingSectionCount = 2)
    {
